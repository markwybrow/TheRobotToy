...
const FormRobotControlPanel = props => {
	...
	const moveRobot = m => {
		const results = moveCommand(
			{ ...gridPosition, ...gridPositionSettings },
			m,
		);
				...
				<div className="text-center">
					<DirectionalPointer>
						<h1>
							...
							<FaPlayCircle
								className={`${getDirection(direction)}Icon`}
								size="2em"
							/>
						</h1>
					</DirectionalPointer>
			...
			<div className="form-group">
				...
				<div className="form-row">
					...
					<a
						...
						id="turnLeft"
						onClick={() => turnRobot('L', gridPosition.direction)}>
						<span className="panel-icon">
							{' '}
					...
					<a
						className="button is-light"
						onClick={() => turnRobot('R', gridPosition.direction)}>
						{' '}
						Right 90%{' '}
						<span className="panel-icon">
							{' '}
