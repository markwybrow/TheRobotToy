...
const Board = () => {
  ...
  const [boxes, setBoxes] = useState(<div className="box" />);
  ...
  useEffect(
    () => {
      ...
      for (let i = 0; i < gridXrow * gridYcolumn; i++) {
        boxCollection.push(
          <div key={`box${i}`} className={`box ${robotPos - 1 === i ? 'active ' + newDirection : ''}`}>
...
const BoardStyle = styled.div`
  ...
  .grid {
    ...
    display: grid;
    grid-template-columns: repeat(${(props) => props.gridXrow}, minmax(2rem, 1fr));
    grid-template-rows: repeat(${(props) => props.gridYcolumn}, minmax(2rem, 1fr));
